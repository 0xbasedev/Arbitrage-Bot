...
import { WETH_ADDRESS, FLASHLOAN_PROVIDER, USE_FLASHLOAN } from "./addresses";
...
export class Arbitrage {
    ...
    for (const tokenAddress in marketsByToken) {
      ...
      for (const m1 of markets) {
        for (const m2 of markets) {
          if (m2 !== m1 && m2.getTokensOut(WETH_ADDRESS, tokenAddress, ETHER.div(100)).gt(
              m1.getTokensIn(tokenAddress, WETH_ADDRESS, ETHER.div(100))
            )) {
  ...
  async takeCrossedMarkets(
    ...
    for (const bestCrossedMarket of bestCrossedMarkets) {
      console.log("Send", bestCrossedMarket.volume.toString(), "WETH for profit", bestCrossedMarket.profit.toString());
      const buyCalls = await bestCrossedMarket.buyFromMarket.sellTokensToNextMarket(
        WETH_ADDRESS, bestCrossedMarket.volume, bestCrossedMarket.sellToMarket);
      const inter = bestCrossedMarket.buyFromMarket.getTokensOut(
        WETH_ADDRESS, bestCrossedMarket.tokenAddress, bestCrossedMarket.volume);
      ...
      // FLASHLOAN OPTIONALITY
      if (USE_FLASHLOAN && !!FLASHLOAN_PROVIDER) {
        // TODO: Add flashloan-enabled contract/call logic here for supported chains/providers
        // For now: skip, as per capital-mode
      }
